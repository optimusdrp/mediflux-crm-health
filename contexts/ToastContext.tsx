          ...
          {toasts.map((t) => {
            ...
            return (
              <motion.div
                ...
                className={`pointer-events-auto flex items-start gap-3 p-4 rounded-xl border shadow-lg backdrop-blur-sm ${borderMap[t.type]}`}
                id={`toast-${t.id}`}
              >
                {iconMap[t.type]}
                <div className="flex-1 text-sm">
                  <div className="font-semibold">{t.title}</div>
                  {t.message && <div className="mt-1 text-xs opacity-90 leading-relaxed">{t.message}</div>}
                </div>
                <button
